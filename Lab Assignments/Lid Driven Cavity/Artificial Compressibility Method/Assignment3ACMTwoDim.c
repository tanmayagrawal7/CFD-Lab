...
int main (void)
{
	double u[grid][grid+1], un[grid][grid+1], uc[grid][grid+1];
	double v[grid+1][grid], vn[grid+1][grid], vc[grid+1][grid];
	double p[grid+1][grid+1], pn[grid+1][grid+1], pc[grid+1][grid+1];
	...
	while (error > 0.0000001)
	{
		...
		for (i=1; i<=(grid-1); i++)
		{
			for (j=1; j<=(grid-1); j++)
			{
				m[i][j] = dy*(  ( un[i][j]-un[i-1][j] ) + ( vn[i][j]-vn[i][j-1] )  );
